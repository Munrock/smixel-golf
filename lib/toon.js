...
class Toon {
    ...
    static async make(data, playerId) {
        ...
        newToonData.deck.push('6046e75541825e2e4c7fc295'); //putter

        await newToonData.save();
        const reloadedToonData = await ToonData.findById(newToonData._id).populate('deck');
        return new Toon(reloadedToonData);
    }

    static async getFromPlayerId(playerId) {
        const toon = await ToonData.findOne({ owner: playerId }).populate('deck').exec();
        if (!!toon) {
    ...
    async destroy() {
        try {
            await ToonData.findOneAndRemove({ _id: this.data._id }).exec();
            return true;
        } catch (e) {
            console.error(e);
            return false;

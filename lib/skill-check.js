class SkillCheck {
    ...
    static explainDice = dice => dice.map(die => `${die.die.map(die=>`d${die}`).join(', ')} from ${die.source}`).join(',\r\n')+'.';
    ...
    test(attributes, target) {
        ...
        attributes.forEach(attribute => {
            let dice = dice.concat(this.findAttributeDice(attribute));
    ...
    findAttributeDice(attr) {
        ...
        categories.forEach(cat => {
            ...
            dice = [{die: this.toon.data.stats.current[cat][attr], source:`${attr} attribute`}];
        });
        return dice;
    }
    ...
    findCardDice(attr) {
        ...
        handIndexes.forEach(index=>{
            ...
            const usableSlots = card.slots.filter(slot=>!usedSlots.includes(slot).length);

class SkillCheck {
    ...
    test(attributes, target) {
        ...
        attributes.forEach(attribute => {
            const possibleDice = this.findAttributeDice(attribute);
            if (SkillCheck.dPower(possibleDice) >= SkillCheck.dPower(dice))
                dice = possibleDice;
        });
        dice = dice.concat(this.findCardDice(attributes));
    ...
    findCardDice(attrs) {
        ...
        handIndexes.forEach(index => {
            ...
            const cardHasAttribute = attrs.some(attr=> tags.includes(attr));
